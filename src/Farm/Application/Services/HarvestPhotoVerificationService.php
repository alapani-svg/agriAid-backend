...
/**
 * Verifies a farmer's declared harvest quantity against the uploaded photo.
 *
 ...
 */
class HarvestPhotoVerificationService
{
    private const TOLERANCE = 0.25;

    /** Expected color hues per crop type (0-360 HSV hue range). */
    private const CROP_COLORS = [
        'maize' => [40, 70],        // yellow-gold
        'cassava' => [20, 50],      // brown-tan
        'yam' => [25, 55],          // brown
        'rice' => [40, 65],         // pale yellow
        'cocoa' => [10, 40],        // brown-orange
        'coffee' => [10, 35],       // dark brown
        'groundnut' => [30, 55],    // tan
        'beans' => [0, 60],         // varied (green/yellow/red)
        'plantain' => [50, 90],     // yellow-green
        'tomato' => [340, 20],      // red
        'pepper' => [340, 20],      // red
        'onion' => [40, 70],        // golden
        'potato' => [25, 55],       // brown-tan
        'sorghum' => [30, 60],      // tan
        'millet' => [35, 65],       // golden
    ];

    /** Plausible harvest quantity ranges per crop (kg). */
    private const CROP_RANGES = [
        'maize' => ['min' => 50, 'max' => 5000, 'median' => 800],
        'cassava' => ['min' => 100, 'max' => 8000, 'median' => 1200],
        'yam' => ['min' => 50, 'max' => 3000, 'median' => 500],
        'rice' => ['min' => 50, 'max' => 4000, 'median' => 600],
        'cocoa' => ['min' => 20, 'max' => 2000, 'median' => 300],
        'coffee' => ['min' => 20, 'max' => 1500, 'median' => 250],
        'groundnut' => ['min' => 30, 'max' => 2000, 'median' => 400],
        'beans' => ['min' => 30, 'max' => 2500, 'median' => 500],
        'plantain' => ['min' => 50, 'max' => 3000, 'median' => 600],
        'tomato' => ['min' => 20, 'max' => 1500, 'median' => 300],
        'pepper' => ['min' => 10, 'max' => 800, 'median' => 150],
        'onion' => ['min' => 20, 'max' => 1200, 'median' => 300],
        'potato' => ['min' => 50, 'max' => 4000, 'median' => 700],
        'sorghum' => ['min' => 50, 'max' => 3000, 'median' => 500],
        'millet' => ['min' => 30, 'max' => 2000, 'median' => 400],
    ];

    private const DEFAULT_RANGE = ['min' => 10, 'max' => 5000, 'median' => 500];
    ...
    public function analyze(string $storagePath, string $cropType, float $declaredQuantityKg): array
    {
        ...
        try {
            ...
            $response = Http::withToken($apiKey)
                ...
                ->post('https://api.openai.com/v1/chat/completions', [
                    ...
                    'messages' => [
                        [
                            'role' => 'system',
                            'content' => 'You are an agricultural inspector estimating harvested crop quantity from a photo. '
                        ...
                        [
                            ...
                            'content' => [
                                [
                                    'type' => 'text',
                                    'text' => sprintf(
                                        'Crop type: %s. The farmer declares %.2f kg. Estimate the quantity of %s '
                                        . 'visible in this photo, in kilograms, and briefly justify your estimate.',
                                        $cropType,
                                        $declaredQuantityKg,
                                        $cropType,
                                    ),
            ...
            if (!$response->successful()) {
                Log::warning('Harvest photo verification: OpenAI request failed, falling back to local algorithm', [
            ...
            Log::error('Harvest photo verification failed, falling back to local algorithm', ['error' => $e->getMessage()]);
     ...
     *   4. EXIF date match     — photo date vs declared harvest   (0–15 pts)
    ...
    private function localAlgorithm(string $storagePath, string $cropType, float $declaredQuantityKg): array
    {
        ...
        // ── 2. Shannon entropy ─────────────────────────────────────
        // Real photos of produce have high entropy (7.0–8.0 bits/byte).
        // Blank/solid-color images have low entropy (< 4.0).
        ...
        if ($exif !== null) {
            ...
            if ($make || $model) {
                ...
                // Has timestamp but no camera make — likely a phone or processed photo
                $cameraScore = 10;
        ...
        if ($photoDate !== null) {
            try {
                ...
                $harvestTimestamp = strtotime(now()->format('Y-m-d'));
                $dateDiffDays = abs($photoTimestamp - $harvestTimestamp) / 86400;

                if ($dateDiffDays <= 1) {
                    $dateScore = 15;
                    $checks[] = sprintf('photo date matches today (%s)', $photoDate);
                } elseif ($dateDiffDays <= 7) {
                    $dateScore = 10;
                    ...
                    $dateScore = 0;
                    $checks[] = sprintf('photo is old (%d days) — may not be current harvest', (int) $dateDiffDays);
                }
            } catch (\Throwable $e) {
                $dateScore = 0;
            }
        ...
        } elseif ($declaredQuantityKg >= $range['min'] && $declaredQuantityKg <= $range['max']) {
            // Within range — score based on how close to the median
        ...
        // Produce an estimated quantity based on the score and crop median
    ...
    {
        // Read first 64 KB for performance — enough to measure entropy
    ...
    private function safeReadExif(string $filePath, string $mime): ?array
    {
        ...
        // Flatten the EXIF array (exif_read_data returns sections)
        $flat = [];
     ...
     * Higher score = more trust in the farmer's declaration.
     */
    private function estimateQuantity(int $score, float $declaredKg, array $range): float
    {
        $trustFactor = match (true) {
            $score >= 80 => 1.0,   // Full trust
            $score >= 70 => 0.95,  // Near-full trust
            $score >= 50 => 0.80,  // High partial trust
            $score >= 40 => 0.65,  // Moderate partial trust
            default => 0.40,       // Low trust — lean toward median
        };

        $estimated = ($declaredKg * $trustFactor) + ($range['median'] * (1 - $trustFactor));

        // Clamp to the crop range
