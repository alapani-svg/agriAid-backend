...
/**
 * Uses an AI vision model to sanity-check a farmer's declared harvest
 * quantity against a photo of the produce, so obviously dishonest
 * declarations get flagged for manual review instead of sailing straight
 ...
 * 'unavailable' rather than blocking harvest recording.
 */
    ...
    public function analyze(string $storagePath, string $cropType, float $declaredQuantityKg): array
    {
        ...
        try {
            ...
            if (!$response->successful()) {
                Log::warning('Harvest photo verification: OpenAI request failed', [
            ...
            Log::error('Harvest photo verification failed', ['error' => $e->getMessage()]);
